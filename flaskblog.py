...
from create_db import db_password
...
try:
    ...
    print("Error connecting to the userdb DB: " + str(error))   # in case of getting the error ' Authentication plugin 'caching_sha2_password' is not supported' uninstall mysql-connector-python and then install it again

db_cursor = mydb.cursor(buffered=True)

try:
    get_score_names_query = 'SELECT name from userdb.xmltable'
    db_cursor.execute(get_score_names_query)
    charlie_parker_scores = db_cursor.fetchall()
except Exception as error:
...
class RegisterForm(FlaskForm):
     ...
     username = StringField('username', validators=[InputRequired(),Length(min=3,max=15)])
     password = PasswordField('password', validators=[InputRequired(), Length(min=8,max=80)])
     # creation_time = PasswordField('password',validators=[InputRequired(), Length(min=8,max=80)])
...
app.config['SQLALCHEMY_DATABASE_URI'] = 'mysql://root:' + str(db_password) + '@localhost/userdb'
...
def signinPost():
    form = RegisterForm()
    print('signin post')
    if form and form.valid_on_submit():
